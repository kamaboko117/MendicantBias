...
const toHHMMSS = (numSecs: string) => {
  ...
  let hours = Math.floor(secNum / 3600)
    .toString()
    .padStart(2, "0");
  let minutes = Math.floor((secNum - Number(hours) * 3600) / 60)
    .toString()
    .padStart(2, "0");
  let seconds = (secNum - Number(hours) * 3600 - Number(minutes) * 60)
    .toString()
    .padStart(2, "0");
...
export function getQueueMessage(queue: any[], index: number, mendicant: Mendicant) : InteractionUpdateOptions {
  let maxItems = 8;
...
export default {
  ...
  async execute(interaction: GuildCommandInteraction, mendicant: Mendicant) {
    ...
    let message = getQueueMessage(queue, 0, mendicant) as InteractionReplyOptions;
